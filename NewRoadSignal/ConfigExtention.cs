...
namespace BveEx.Toukaitetudou.RoadSignal
{
    public static class ConfigExtention
    {
        ...
        {
            if (!File.Exists(Path.Combine(filename, structure.OnLightStructure)))
            {
            ...
            bool isOffExists = !(structure.OffLightStructure is null) &&!File.Exists(Path.Combine(filename, structure.OffLightStructure));
            if (isOffExists)
            {
